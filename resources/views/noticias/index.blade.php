...
@section('content')
<section class="font-display bg-background-light dark:bg-background-dark text-[#1A1A1A] dark:text-gray-200">
    ...
    <div class="px-4 sm:px-8 md:px-16 lg:px-24 xl:px-40 flex flex-1 justify-center py-5">
      <div class="layout-content-container flex flex-col max-w-[960px] flex-1">
        ...
        <div class="flex flex-wrap justify-between gap-3 p-4">
          <h1 class="text-4xl font-black leading-tight tracking-[-0.033em] min-w-72 text-[#1A1A1A] dark:text-white">
            ...
            <a
              ...
              class="h-8 shrink-0 inline-flex items-center justify-center gap-x-2 rounded-full px-4 text-sm font-medium leading-normal
                {{ $isActive
                  ...
                  : 'bg-gray-200 dark:bg-gray-700 text-[#555555] dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors' }}">
              {{ $cat }}
            </a>
          @endforeach
        </div>
        ...
        @else
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
            @foreach($noticias as $noticia)
              <article
                class="flex flex-col gap-3 rounded-xl overflow-hidden bg-white dark:bg-background-dark shadow-sm hover:shadow-lg transition-shadow duration-300 group">
                ...
                <div class="p-4 flex flex-col flex-1">
                  <p class="text-[#555555] dark:text-gray-400 text-xs font-normal leading-normal">
                    {{ $noticia->fecha_publicacion_formateada }}
                  </p>
                  <h3 class="text-[#1A1A1A] dark:text-white text-lg font-bold leading-tight my-1 flex-1 line-clamp-2">
                    {{ $noticia->titulo }}
                  </h3>
                  @if($noticia->bajada)
